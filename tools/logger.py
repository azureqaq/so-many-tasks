...
# 格式化器
# 终端输出
__con_ft = logging.Formatter(fmt=r'%(asctime)s|%(levelname)-8s|%(filename)s:[%(lineno)3s]| %(message)s', datefmt=r'%H:%M:%S')
# 文件输出
__file_ft = logging.Formatter(fmt=r'%(asctime)s|%(levelname)-8s|%(filename)s:[%(lineno)3s]| %(message)s', datefmt=r'%Y-%m-%d %H:%M:%S')
